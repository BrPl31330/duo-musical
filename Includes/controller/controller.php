...
class Controller
{
    ...
    static function traiterFormulaire()
    {
        if (!empty($_POST)) {
            ...
            $tableauAsso = [
                "titre"         => Controller::filtrerTexte("titre"),
                "contenu"       => Controller::filtrerTexte("contenu"),
                "image"       => Controller::filtrerUpload("image"),
                "slug"          => Controller::filtrerTexte("slug"),
                "categorie"      => Controller::filtrerTexte("categorie"),
                "priorite"      => Controller::filtrerTexte("priorite"),
                "creer_le"    => date("Y-m-d H:i:s"),    // remplissage automatique avec la date actuelle
            ];
    ...
    static function traiterPassword ()
    {
                if (!empty($_POST)) {
            //  infos de formulaire
            // AJOUTER FILTRES SECURITE
            $tableauAsso = [
                "nom"            => Controller::filtrerTexte("nom"),
                "password"       => Controller::filtrerTexte("password"),
                "date_entree"     => date("Y-m-d H:i:s"), //  date actuelle
                // "rgpd"           => Controller::filtrerTexte("rgpd"), ne pas mettre sinon défaut !!!
            ];


            // tester si le tableau d'erreur est vide
            if (empty(Controller::$tabErreur)) {
                // appelle methode pour inserer dans SQL
                Model::insererUser($tableauAsso);

                // DEBUG
                echo "<strong>Bienvenue cher membre</strong>";
    ...
    //             "nom"            => Controller::filtrerTexte("nom"),
    //             "password"       => Controller::filtrerTexte("password"),
    //             "dateEntree"     => date("Y-m-d H:i:s"), //  date actuelle
    //             "rgpd"           => Controller::filtrerTexte("rgpd"),
    //         ];
